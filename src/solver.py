...
class TreeSolver(Solver):
    ...
    def solve(self, fth: qu.FidType=0.90, cost_cap=0, attr: str='adjust_eff'):
        ...
        while self.tree.root.fid < fth and self.tree.root.cost < cost_cap:
            ...
            f, p = self.tree.gate.purify(node.fid, node.fid)
            dc = (f - node.fid)*node.grad_cf + node.cost*node.grad_cn
            c = node.cost + dc
...
class GRDSolver(Solver):
    ...
    def seq_purify(self, fids: list[qu.FidType], allocs: list[int]) -> 'list[qu.FidType]':
        pfids = []
        for i in range(len(fids)):
            pfids.append(self.gate.seq_purify([fids[i]] * allocs[i]))
        return pfids
    
    def balanced_purify(self, fids: list[qu.FidType], allocs: list[int]) -> 'list[qu.FidType]':
        assert False, "not implemented"

    def swap_purify(self, fids: list[qu.FidType], allocs: list[int]) -> qu.FidType:
        pfids = self.seq_purify(fids, allocs)
    ...
    def solve(self, fth: qu.FidType, cost_cap: qu.ExpCostType):
        ...
        for edge in self.edges:
            self.alloc[edge] = 1
        pfids = self.seq_purify(self.fids, list(self.alloc.values()))
