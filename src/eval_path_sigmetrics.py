...
def test_dp_sys_2(fth, exp_num):
    ...
    ylabel = "Cost"
    markers = ['o', 's', 'v', 'd', 'p', 'x', 'h', '>', '<']
    filename = "../data/path/dp_path_cost_f={}_2.png".format(fth)
    draw_lines(x, ys, xlabel, ylabel, labels, markers, filename=filename, yscale='log')

    ys = times.T
    ylabel = "Time (s)"
    filename = "../data/path/dp_path_time_f={}_2.png".format(fth)
    draw_lines(x, ys, xlabel, ylabel, labels, markers, filename=filename, yscale='log')

    ys = fids.T
    ylabel = "Fidelity"
    filename = "../data/path/dp_path_fid_f={}_2.png".format(fth)
    draw_lines(x, ys, xlabel, ylabel, labels, markers, filename=filename, ylim=(0.5, 1))
